...
- **Cross-model (required when `codex` is installed)** — [../requesting-code-review/codex-adversarial-review.md](../requesting-code-review/codex-adversarial-review.md). A Codex (different-model) adversarial pass over the plan **with its originating design**: `codex-review --kind plan --file <plan> --design <spec>`. Passing `--design` is what makes the coverage check real — Codex compares plan against design (every goal / acceptance criterion / non-goal honored, no gold-plating) on top of executability; without it Codex never sees the design and won't assess coverage. The strongest *independent* lens — a different model has different blind spots. Run it whenever `codex` is on `PATH`; it is skippable **only** when `codex` is absent (it degrades to a skip).
